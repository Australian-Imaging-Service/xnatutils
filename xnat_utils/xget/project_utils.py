...
def id_to_label(conn,proj):
    ...
    if not proj:
        ...
        try:
            return conn.select.project(proj).xpath("/xnat:Project/xnat:name")[0].text
        except Exception:
            return None
...
def choose_single(conn, projs,label):
    ...
    """
    if len(projs) > 1:
        raise Utils.AmbiguousLabelError("More than one project ID associated with name " + str(label) + ": "  + ",".join(projs))
    elif not projs:
        return None
    else:
        return projs[0]
...
def projects_matching_label(conn,label):
    ...
    if not label:
        ...
        def exact_match(p):
            ...
            """
            return conn.select.project(p).xpath("/xnat:Project/xnat:name")[0].text == label
...
def label_id_flip(conn,proj):
    ...
    if label:
        if conn.select.project(proj).exists() and id_to_label(conn,proj) != proj:
            raise Utils.AmbiguousLabelError("The given label " + proj + " is both a name and an ID.")
        else:
            return label
    else:
        if conn.select.project(proj).exists():
