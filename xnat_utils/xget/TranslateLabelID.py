...
def in_project(e, p):
    ...
    shared_xpath = e.xpath(
        "/*/xnat:sharing/xnat:share/@project",
        namespaces=e.nsmap)
...
class Project():
    ...
    def get(self, proj_id):
        if proj_id not in self.xml:
            try:
                self.xml[proj_id] = etree.fromstring(
                    self.conn.select.project(proj_id).get())
    ...
    def search_label(self, label):
        ...
        res = self.conn.select(
            'xnat:projectData',
            ['xnat:projectData/ID']).where(constraints).get("id")
    ...
    def exists(self, proj_id):
        try:
            if self.xpath(self.get(proj_id), "@ID"):
                return True
        except Exception as e:
            return False
...
class Scan():
    ...
    def has_quality(self, quality):
        qs = [
            x.text for x in self.xpath(
                self.exp_xml,
                "/*/xnat:scans/xnat:scan/xnat:quality")]
...
class Assessor():
    ...
    def get_type(self, ass_type):
        ...
        if ass_type != "ALL":
            for e in self.xpath(
                    self.exp_xml, "/*/xnat:assessors/xnat:assessor"):
                if e.attrib[
                        '{http://www.w3.org/2001/XMLSchema-instance}type'] == ass_type:
                    ret.append(e.attrib['ID'])
        elif ass_type == "ALL":
            return self.get_ids()
        return ret
...
class Reconstruction():
    ...
    def get_ids(self):
        ret = []
        for e in self.xpath(
                self.exp_xml, "/*/xnat:reconstructions/xnat:reconstructedImage"):
            ret.append(e.attrib['ID'])
        return ret
    ...
    def get_type(self, recon_type):
        ...
        if recon_type != "ALL":
            for e in self.xpath(
                    self.exp_xml, "/*/xnat:reconstructions/xnat:reconstructedImage"):
                if e.attrib[
                        '{http://www.w3.org/2001/XMLSchema-instance}type'] == recon_type:
                    ret.append(e.attrib['ID'])
        elif recon_type == "ALL":
            return self.get_ids()
        return ret
...
def exists(resource, label_or_id, choose=None):
    if resource.exists(label_or_id):
        ...
        if not choose:
            return label_to_id(
                resource, label_or_id, choose_single(label_or_id)) is not None
        else:
