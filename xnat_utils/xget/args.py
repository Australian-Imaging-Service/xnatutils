...
parser = argparse.ArgumentParser(description="Download files from an XNAT Instance - TODO make this better.");
...
parser.add_argument("-start",nargs='?',type=str,help="Retrieve experiments created on and AFTER this date. Uses DAYMONTHYEAR format eg.01012011 is January 1st 2010.")
parser.add_argument("-end",nargs='?',type=str,help="Retrieve experiments created on and BEFORE this date. Uses DAYMONTHYEAR format eg.01012011 is January 1st 2010.")
...
def argTry(check) :
    if not check['result']:
        sys.stderr.write("Illegal/Missing arguments: Required args : " + check['log']+ "\n")
        sys.exit()
...
def getAllBut (argGroups, arg) :
    return filter(lambda group: group['result'] and (group['needed'] != arg['needed']), argGroups)
...
def someGroups(argGroups):
    return (not allGroups(argGroups,False)) or (not allGroups(argGroups,True))
...
def ppFlagsTest():
    ...
    groups = [{"result": True , "missing": [], "needed": ["a"]},{"result":True , "missing": [], "needed": ["u", "p"]}]
...
def argGroupTest():
    ...
    assert(argGroup({'p': "testPassword", 'u':"testUser"}, ['u','p']) == {'result': True, 'missing':[], 'needed':["u","p"]})
    assert(argGroup({'p': None ,          'u':"testUser"}, ['u','p']) == {'result': False, 'missing':["p"], 'needed':["u","p"]})
    assert(argGroup({'p': None ,          'u': None     }, ['u','p']) == {'result': False, 'needed': ['u','p'],  'missing': ['p', 'u']})
...
def consolidateLogTest():
    ...
    assert(consolidateLogs(testResults) == "Error One , Error Two , Warning One , Warning Two , ");
