...
class Project():
    ...
    def getSessions(self):
        def get_from_xnat(conn):
            ...
            byLabel = conn.select.project(
                SessionUtils.label_to_id(
                    conn,
...
class Model(Auth, Connection):
    ...
    def getHost(self):
        if self.connection._available():
            ...
            sys.stderr.write(
                "No host specified on command line or in the XNAT pass file\n")
            sys.exit
    ...
    def getUser(self):
        if self.auth._available():
            ...
            sys.stderr.write(
                "No user specified on command line or in the XNAT pass file\n")
            sys.exit()
    ...
    def getPass(self):
        if self.auth._available():
            ...
            sys.stderr.write(
                "No password specified on command line or in the XNAT pass file\n")
            sys.exit()
