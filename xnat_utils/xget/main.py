...
def main():
    ...
    not os.path.exists(opts.getOutputDir()) and \
        Utils.fatal_error("Output directory %s does not exist" % (opts.getOutputDir()))
    not os.access(opts.getOutputDir(), os.W_OK) and \
        Utils.fatal_error("Output directory %s is not writeable" % (opts.getOutputDir()))
    
    Utils.print_info("Connecting to XNAT Server %s" % (opts.getHost()))
    connection = createConnection(opts.getHost(), opts.getUser(), opts.getPass(), opts.getOutputDir())
    if DEBUG:
    ...
    elif isinstance(err,errors.OperationalError):
    ...
    Utils.validDateRange(start,end) or Utils.fatal_error("%s - %s is not a valid date range" % (start,end))
    ...
    if proj:
         Utils.print_info("Checking that project %s exists." % (opts.getProject()))
         if not exists(proj_resource,proj):
              Utils.fatal_error("Project %s does not exist." % (proj))
         Utils.print_info("Checking that the project %s is unique in the XNAT server" % (proj))     
         ...
         if _proj and _proj != proj:
              Utils.print_info("Project label %s has been translated to project id %s" % (proj,_proj))
              proj = _proj

    if proj:
         Utils.print_info("Checking that the experiments %s exist within project %s" % (",".join(opts.getSessions()), proj))
    else:
         Utils.print_info("Checking that the specified experiments %s exist." % (opts.getSessions()))
    ...
    sessions = filter(lambda (s,sid): Utils.inRange(start,end,exp_resource.date(sid)),
    ...
    for s in candidates:
         if not candidates[s]['scan']['list']:
              Utils.print_warning("No acquisitions found in experiment %s matching the constraints." % (s))
         ...
         if not candidates[s]['assessor']['list']:
              Utils.print_warning("No assessors found in experiment %s matching the constraints." % (s))
         ...
         if not candidates[s]['recon']['list']:
              Utils.print_warning("No reconstructions found in experiment %s matching the constraints." % (s))
...
def sanity_check_filename(filename,outputdir,overwrite):
     ...
     if os.path.exists(location) and not overwrite:
          return Exception("Cannot download to %s because it already exists. Specify another output directory or set the -overwrite flag." % (location))
     else:
          return filename
     
def pp_download_candidates(candidates):
    s = "Download Candidates:\n"
    pp_list = lambda c,t:not isinstance(candidates[c][t]['filename'],Exception) and str(candidates[c][t]['list']) or str([])
    for c in candidates:
         s += "         " + c + ":\n"
...
def create_filename(filters,longname,proj,quality,format,downloadtype,s):
     ...
     if downloadtype == 'scan':
          if longname:
               xs.append("acq_%s_q_%s_f_%s" % ("_".join(filters['scan']),quality,format))
          else:
               xs.append(s+"_acq")
...
def sanity_check_project(proj_resource,proj):
     if proj:
          ...
          except AmbiguousLabelError,e:
               return e
          
def sanity_check_filter(constraints, constraint_type):
     if len(constraints) > 1 and 'ALL' in constraints:
        return ('Error in %s constraint: \"ALL\" cannot be used with any other constraint' % (constraint_type))
...
def create_session_dict(exp, sessions, proj=None):
    def conv(s):
        ...
        if proj:
             if not in_proj():
                  return LookupError("Experiment %s either doesn't exist in project %s or in the XNAT server" % (s,proj))
             else:
                  return True
        elif not in_xnat():
...
def download_images(candidates, opts,conn):
     for s in candidates:
          ...
          for t in ['scan','assessor','recon']:
               if candidates[s][t]['list'] and not isinstance(candidates[s][t]['filename'],Exception):
                    ...
                    try : 
                         ...
                         if opts.extractZip():
                              ...
                              try : 
                                   paths =  Utils.extractZip(zip_location,opts.overwrite(),opts.getOutputDir())
                                   Utils.print_info("Extracted to : " + str(paths))
                                   Utils.print_info("Removing zip file : " + str(zip_location))
