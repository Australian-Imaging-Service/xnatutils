    ...
    **kwargs,
    ...
    with connect(**kwargs) as login:
        ...
        for session in matched_sessions:
            for scan in matching_scans(session, scans, match_id=match_scan_id):
                resources = []
                suffix = False
                if resource_name is not None:
                    ...
                    resource_names = [
                        r.label
                        for r in scan.resources.values()
    ...
    if not downloaded_resources:
        logger.warning(
            ("No scans matched pattern(s) '%s' in specified " "sessions (%s)"),
            "', '".join(scans) if scans is not None else "",
            "', '".join(s.label for s in matched_sessions),
        )
    else:
        num_resources = reduce(add, map(len, downloaded_resources.values()))
        logger.info(
            "Successfully downloaded %s scans from %s session(s)",
            num_resources,
    ...
    **kwargs,
    ...
    with connect(**kwargs) as login:
        for entry in root.iter("{http://nrg.wustl.edu/catalog}entry"):
            ...
            if "scans" in uri:
                scan = login.create_object(re.match(r".*/scans/[^/]+", uri).group(0))
            else:
                scan = None
            _download_resource(
                resource,
...
def _download_resource(
    ...
    if subject_dirs:
        target_dir = os.path.join(
            download_dir, _get_subject_from_session(session).label
        )
    else:
        ...
        try:
            ...
            try:
                ...
                raise XnatUtilsUsageError(
                    "Cannot convert to unrecognised format '{}'".format(convert_to)
                ) from e
    else:
        target_ext = ""
    ...
    try:
        ...
        try:
            status = int(re.match(r".*\(status (\d+)\).*", str(e)).group(1))
            if status == 404:
                logger.warning(
                    "Did not find any files for resource '%s' in '%s' " "session",
                    resource.label,
                    session.label,
                )
                return True
    ...
    if (
        ...
        if convert_to is not None:
            if converter is None:
                if convert_to in ("nifti", "nifti_gz") and resource.label == "DICOM":
                    converter = "dcm2niix"
                else:
                    converter = "mrconvert"
        ...
        try:
            if converter == "dcm2niix":
                ...
                sp.check_call(
                    '{} "{}" "{}"'.format(mrconvert, src_path, target_path), shell=True
                )
            else:
                if resource.label == "DICOM" and convert_to in ("nifti", "nifti_gz"):
                    msg = "either dcm2niix or "
                else:
                    msg = ""
                raise XnatUtilsUsageError(
                    "Please install {} mrconvert to convert between {}"
                    "and {} formats".format(msg, resource.label.lower(), convert_to)
                )
...
def parser():
    ...
    parser.add_argument(
        "session_or_regex_or_xml_file",
        type=str,
        nargs="*",
        help=(
            "Name or regular expression of the session(s) "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to download the scans to. If not provided "
    ...
    parser.add_argument(
        ...
        nargs="+",
        help=(
            "Name of the scans to include in the download. "
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The name of the resource to download. Not "
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="+",
        help=("Only download from sessions containing the " "specified scans"),
    )
    parser.add_argument(
        "--without_scans",
        "-o",
        ...
        default=None,
        nargs="+",
        help=("Only download from sessions that don't contain " "the specified scans"),
    )
    parser.add_argument(
        "--convert_to",
        "-c",
        ...
        help=(
            "Runs a conversion script on the downloaded "
    ...
    parser.add_argument(
        ...
        choices=converter_choices,
        help=(
            "The conversion tool to convert the downloaded "
            ...
            "mrconvert for other conversions".format("', '".join(converter_choices))
        ),
    )
    parser.add_argument(
        "--subject_dirs",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether to ignore previously downloaded "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Only select sessions before this date "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Only select sessions after this date " "(in Y-m-d format, e.g. 2018-02-27)"
        ),
    )
    parser.add_argument(
        "--project",
        ...
        type=str,
        default=None,
        help=("The ID of the project to list the sessions " "from."),
    )
    parser.add_argument(
        "--subject",
        "-j",
        ...
        default=None,
        help=(
            "The ID of the subject to list the sessions "
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help=("To disable matching on scan ID if the scan " "type is None"),
    )
    parser.add_argument(
        "--strip_name",
        "-i",
...
def cmd(argv=sys.argv[1:]):
    ...
    try:
        if len(
            args.session_or_regex_or_xml_file
        ) == 1 and args.session_or_regex_or_xml_file[0].endswith(".xml"):
            get_from_xml(
                args.session_or_regex_or_xml_file[0],
                download_dir,
